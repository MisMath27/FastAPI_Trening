...
class TestUsersCRUD:
    ...
    async def test_create_user_success(self, async_client: AsyncClient, test_user_data):
        ...
        # НЕ проверяем age (его нет в ответе)
    ...
    async def test_create_user_with_faker_data(self, async_client: AsyncClient, faker):
        ...
        user_data = {
            ...
            "age": faker.random_int(min=19, max=119),
            "email": faker.email(),           # Добавлен email
            "password": faker.password(length=10)  # Добавлен password
        }
    ...
    async def test_create_user_boundary_age(self, async_client: AsyncClient, faker):
        ...
        # Минимальный валидный возраст = 19
        user_data_min = {
            "username": faker.user_name(),
            "age": 19,
            "email": faker.email(),
        ...
        # Максимальный валидный возраст = 119
        user_data_max = {
            "username": faker.user_name(),
            "age": 119,
            "email": faker.email(),
    ...
    async def test_user_data_types(self, async_client: AsyncClient, test_user_data):
        ...
        assert isinstance(data["email"], str)  # ← проверяем email
        # НЕ проверяем age (его нет в ответе)
